...
func (r *PostgresCashBalanceRepository) Upsert(balance *domain.CashBalance) error {
	query := `
		INSERT INTO investments.cash_balances (user_id, currency, balance, updated_at)
		VALUES ($1, $2, $3, $4)
		ON CONFLICT (user_id, currency) 
		DO UPDATE SET balance = $3, updated_at = $4
	`
	_, err := r.db.Exec(query, balance.UserID, balance.Currency, balance.Balance, time.Now())
...
func (r *PostgresCashBalanceRepository) AddAmount(userID, currency string, amount float64) error {
	query := `
		INSERT INTO investments.cash_balances (user_id, currency, balance, updated_at)
		VALUES ($1, $2, $3, NOW())
		ON CONFLICT (user_id, currency)
		DO UPDATE SET 
			...
			updated_at = NOW()
	`
	_, err := r.db.Exec(query, userID, currency, amount)
