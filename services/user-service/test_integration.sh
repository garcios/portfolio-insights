...
echo -e "${YELLOW}=== User Service Integration Tests ===${NC}"
echo ""
...
echo -e "${YELLOW}=== Test 1: CreateUser RPC ===${NC}"
...
echo -e "${YELLOW}=== Test 2: GetUser RPC ===${NC}"
...
if response=$(grpcurl -plaintext -import-path ../../proto -proto user/user.proto \
    ...
    RESP_ID=$(echo "$response" | jq -r '.id' 2>/dev/null || echo "")
    RESP_EMAIL=$(echo "$response" | jq -r '.email' 2>/dev/null || echo "")
    RESP_USERNAME=$(echo "$response" | jq -r '.username' 2>/dev/null || echo "")
    
    if [ "$RESP_ID" = "$USER_ID" ] && [ "$RESP_EMAIL" = "$TEST_EMAIL" ] && [ "$RESP_USERNAME" = "$TEST_USERNAME" ]; then
        ...
        echo "  Expected ID: $USER_ID, got: $RESP_ID"
...
echo -e "${YELLOW}=== Test 3: VerifyUser RPC (Valid Credentials) ===${NC}"
...
if response=$(grpcurl -plaintext -import-path ../../proto -proto user/user.proto \
    ...
    if [ "$VALID" = "true" ] && [ "$RESP_ID" = "$USER_ID" ] && [ "$RESP_EMAIL" = "$TEST_EMAIL" ]; then
        echo -e "${GREEN}✓ PASSED${NC}"
        echo "  Valid: $VALID"
        echo "  User ID: $RESP_ID"
        echo "  Email: $RESP_EMAIL"
        echo "  Username: $RESP_USERNAME"
        ((PASSED++))
    else
...
echo -e "${YELLOW}=== Test 4: VerifyUser RPC (Invalid Password) ===${NC}"
...
echo -e "${YELLOW}=== Test 5: VerifyUser RPC (Non-existent User) ===${NC}"
...
echo -e "${YELLOW}=== Test 6: Database Schema Validation ===${NC}"
...
if [ $FAILED -eq 0 ]; then
    ...
    echo ""
    echo "Summary:"
    echo "  ✓ CreateUser RPC working"
    echo "  ✓ GetUser RPC working"
